...
for row in reader2:
    ...
    if unicode(str(row[2]), "utf8") != '-':
        code = unicode(str(row[2]), "utf8")
        name = unicode(str(row[3]), "utf8")
        if code + '-' + name not in state_list:
            record = doc.createElement('record')
            record.setAttribute("id", "city_new_id_" + code + '_' + remove_accents(name).replace(' ', '').lower())
            record.setAttribute("model", "res.country.state.city")

            data_rec_dict = {
                'code': unicode(str(row[2]), "utf8"),
                'name': unicode(str(row[3]), "utf8"),
                'state_id': "country_state_new_id_" + unicode(str(row[0]).replace(".", "_"), "utf8")
            }
            state_list.append(code + '-' + name)
    ...
    if count_row == 23179:
        ...
        if a:
            fo = open("./carga/"+"city_"+str((index_iter/23179))+".xml", "wb")
