...
def obtener_comprobantes_cr(token_cr):
    logging.basicConfig()
    logging.getLogger().setLevel(logging.DEBUG)
    requests_log = logging.getLogger("requests.packages.urllib2")
    requests_log.setLevel(logging.DEBUG)
    ...
    REDIRECT_URI = 'https://api.comprobanteselectronicos.go.cr/recepcion-sandbox/v1/comprobantes'
    ...
    response = requests.get(REDIRECT_URI, headers=headers)
    print response.content
...
response_cr = obtener_comprobantes_cr(token_t)