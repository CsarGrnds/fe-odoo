...
for row in reader2:
    ...
    if unicode(str(row[4]), "utf8") != '-':
        code = unicode(str(row[4]), "utf8")
        name = unicode(str(row[5]), "utf8")
        name_city = unicode(str(row[3]), "utf8")
        code_city = unicode(str(row[2]), "utf8")
        city_id_str = "city_new_id_" + code_city + '_' + remove_accents(name_city).replace(' ', '').lower()
        if code + '-' + name + code_city + '_' + remove_accents(name_city).replace(' ', '').lower() not in state_list:
            record = doc.createElement('record')
            record.setAttribute("id", "district_new_id_"+ code + '_' + remove_accents(name).replace(' ', '').lower())
            record.setAttribute("model", "res.country.district")
            ...
            state_list.append(code + '-' + name + code_city + '_' + remove_accents(name_city).replace(' ', '').lower())
    ...
    if count_row == 23179:
        ...
        if a:
            fo = open("./carga/"+"district_"+str((index_iter/23179))+".xml", "wb")
